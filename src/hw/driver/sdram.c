...
bool sdramInit(void)
{
  ...
  if (ret == true)
  {
    cliPrintf("SDRAM %dMB \t\t: OK\r\n", (int)(SDRAM_DEVICE_SIZE/1024/1024));
  }
  else
  {
    cliPrintf("SDRAM  \t\t: Fail\r\n");
  }
...
#define REFRESH_COUNT                    ((uint32_t)917)   /* SDRAM refresh counter (120Mhz SD clock) */
...
void BSP_SDRAM_Initialization_sequence(uint32_t RefreshCount)
{
  ...
  Command.CommandMode            = FMC_SDRAM_CMD_PALL;    // all Bank Precharging CMD //
  Command.CommandTarget          = FMC_SDRAM_CMD_TARGET_BANK1;
  Command.AutoRefreshNumber      = 1;
  Command.ModeRegisterDefinition = 0;
...
//          }
  ...
  if (ret == false)
  {
    cliPrintf( "sdram write 1~100 \n");
    cliPrintf( "sdram erase \n");
  }
}
#endif
